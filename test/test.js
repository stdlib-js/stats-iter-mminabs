...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof itermminabs, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function throws an error if provided a second argument which is not a positive integer', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			itermminabs( randu(), value );
		};
	}
});

tape( 'the function returns an iterator protocol-compliant object which iteratively computes a moving minimum absolute value', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [ 2.0, 2.0, 2.0, 2.0, 2.0, 3.0 ];

	it = itermminabs( array2iterator( values ), 3 );
...
tape( 'the function returns an iterator protocol-compliant object which iteratively computes a moving minimum absolute value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	it = itermminabs( createIterator( values ), 3 );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'if an iterated value is a non-numeric value, the computed minimum absolute value is `NaN` for at least `W` invocations', function test( t ) {
	var expected;
	var values;
	var it;
	var v;
...
tape( 'if an environment supports `Symbol.iterator` and the provided iterator is iterable, the returned iterator is iterable', function test( t ) {
	...
	itermminabs = proxyquire( './../lib/main.js', {
